...
export default function MealPlan() {
  const {
    bodyProfile, setBodyProfile, bodyAnalysis, pantry, supplements, takeoutDishes,
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const plan = await generateMealPlan(apiKey, bodyProfile, bodyAnalysis, pantry, supplements, takeoutDishes);
      setMealPlan(plan);
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-slate-800 rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${!isNoCook ? 'bg-orange-600 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}>🍳 开火</button>
            <button
              onClick={() => setBodyProfile({ cookingPreference: 'nocook' })}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${isNoCook ? 'bg-blue-600 text-white shadow' : 'text-gray-500 hover:text-gray-300'}`}>🛵 不开火</button>
          </div>
